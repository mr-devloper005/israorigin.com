...
const inputClass = 'h-12 w-full rounded-full border border-[var(--editable-border)] bg-[var(--slot4-surface-bg)] px-5 text-[0.95rem] font-medium text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--iso-green)] focus:ring-4 focus:ring-[var(--iso-green-soft)]'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-full bg-[var(--iso-ink)] px-6 text-sm font-semibold text-[var(--iso-cream)] transition duration-300 hover:-translate-y-0.5 hover:bg-[var(--iso-green)] hover:text-[#12210E] active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.15rem] px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-[var(--iso-green-soft)] text-[var(--iso-green-deep)]' : 'bg-[var(--iso-coral-soft)] text-[#B3341F]'}`}>{message}</p> : null}
